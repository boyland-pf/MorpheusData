...
inner_join(faculty) %>% 
inner_join(student) 
write.csv(input, "data-raw/s1_input1.csv", row.names=FALSE)
s1_input1 <- read.csv("data-raw/s1_input1.csv", check.names = FALSE)
fctr.cols <- sapply(s1_input1, is.factor)
int.cols <- sapply(s1_input1, is.integer)
s1_input1[, fctr.cols] <- sapply(s1_input1[, fctr.cols], as.character)
s1_input1[, int.cols] <- sapply(s1_input1[, int.cols], as.numeric)
save(s1_input1, file = "data/s1_input1.rdata")

output=input%>%filter (F_name == "faculty1" & level == "JR") %>% select(S_name)
write.csv(output, "data-raw/s1_output1.csv", row.names=FALSE)
s1_output1 <- read.csv("data-raw/s1_output1.csv", check.names = FALSE)
fctr.cols <- sapply(s1_output1, is.factor)
int.cols <- sapply(s1_output1, is.integer)
s1_output1[, fctr.cols] <- sapply(s1_output1[, fctr.cols], as.character)
s1_output1[, int.cols] <- sapply(s1_output1[, int.cols], as.numeric)
save(s1_output1, file = "data/s1_output1.rdata")
