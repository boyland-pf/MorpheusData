...
write.csv(input, "data-raw/s17_input1.csv", row.names=FALSE)
s17_input1 <- read.csv("data-raw/s17_input1.csv", check.names = FALSE)
fctr.cols <- sapply(s17_input1, is.factor)
int.cols <- sapply(s17_input1, is.integer)
s17_input1[, fctr.cols] <- sapply(s17_input1[, fctr.cols], as.character)
s17_input1[, int.cols] <- sapply(s17_input1[, int.cols], as.numeric)
save(s17_input1, file = "data/s17_input1.rdata")

df1=input %>% group_by(part_key) %>% summarise(mean = mean(cost))
output= input %>% inner_join(df1) %>% filter(cost > mean) %>% select(part_key,sname)
write.csv(output, "data-raw/s17_output1.csv", row.names=FALSE)
s17_output1 <- read.csv("data-raw/s17_output1.csv", check.names = FALSE)
fctr.cols <- sapply(s17_output1, is.factor)
int.cols <- sapply(s17_output1, is.integer)
s17_output1[, fctr.cols] <- sapply(s17_output1[, fctr.cols], as.character)
s17_output1[, int.cols] <- sapply(s17_output1[, int.cols], as.numeric)
save(s17_output1, file = "data/s17_output1.rdata")
